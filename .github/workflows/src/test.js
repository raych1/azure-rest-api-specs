 ...
 */
export default async function getAzurePipelineArtifact({
  core,
  retryOptions = {},
  fallbackToFailedArtifact = false,
  token = process.env.ADO_TOKEN,
}) {
  // Use provided parameters or fall back to test values
  const ado_build_id = '5719580';
  const ado_project_url = 'https://dev.azure.com/azure-sdk/590cfd2a-581c-4dcb-a12e-6568ce786175';
  const artifactName = 'spec-gen-sdk-artifact';
  const artifactFile = 'spec-gen-sdk-artifact.json';
  ...
  if (token) {
    core.info('Using provided token for authentication');
  } else {
    core.warning('No token provided - request may fail if authentication is required');
  }
  ...
  if (response.ok) {
    ...
    downloadUrl += `?format=file&subPath=/${artifactFile}`;
    ...
    core.info(`Successfully downloaded artifact '${artifactData}'`);
  } else {
